...
@Component({
  ...
  templateUrl: './front-upload.component.html',
  styleUrls: ['./front-upload.component.scss']
})
export class FrontUploadComponent implements OnInit {
  uploadType;
  type; //上传文件类型；
  isCropper; //是否裁切；
  ratio; // 裁切比率；
    ...
    if(file.size < this.size*1024*1024 ){
      ...
      fr.onload =(e)=> {
        ...
        if (/image/g.test(file.type)) {
          let img = $('<img style="width:100%" src="' + result + '">');
          $('.preview1').html('').append(img);
        } else if (/video/g.test(file.type)) {
          let vidoe = $('<video style="width:100%" controls src="' + result + '">');
      ...
      this._message.error('上传文件大小应在500M以内');
